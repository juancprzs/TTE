...
def fbresnet_augmentor(isTrain):
    ...
    """
    if isTrain == True:
        print('USING TRAINING AUGMENTOR')
        augmentors = [
    ...
    elif isTrain == 'wrapper':
        print('USING WRAPPER AUGMENT')
        augmentors = [
            imgaug.ResizeShortestEdge(256, cv2.INTER_CUBIC),
            imgaug.CenterCrop((256, 256)),
        ]
    else:
        print('USING VAL AUGMENTOR')
        augmentors = [
            imgaug.ResizeShortestEdge(256, cv2.INTER_CUBIC),
...
def get_val_dataflow(
    ...
    if augmentors is None or augmentors == 'wrapper':
        augmentors = fbresnet_augmentor(False if augmentors is None else 'wrapper')
...
def eval_on_ILSVRC12(model, sessinit, dataflow, output_path=None):
...
class ImageNetModel(ModelDesc):
    ...
    def inputs(self):
        ...
        return [tf.placeholder(self.image_dtype, [None, 256, 256, 3], 'input'),
                tf.placeholder(tf.int32, [None], 'label')]
