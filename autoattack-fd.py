...
def load_and_process(path, inter=cv2.INTER_CUBIC):
    ...
    if H:
        center_crop = resized[13: 237, (int(w / 2) - 114): (int(w / 2) + 114), :]
    else:
        center_crop = resized[(int(h / 2) - 112): (int(h / 2) + 112), 13: 237, :]
...
def main(args):
    ...
    # forward here
    img_input = tf.placeholder(dtype=tf.float32, shape=[None, 224, 224, 3])
